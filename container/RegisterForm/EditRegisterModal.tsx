...
interface EditRegisterFormModalProps {
    data: RegisterForm;
    apiEdit: () => Promise<any>; // add this line
}
const EditRegisterFormModal: React.FC<EditRegisterFormModalProps> = ({ data, apiEdit }) => {
    ...
    let heading = `ลบข้อมูล : ${data?.regId} ${data.regIdpersonal} 
    ${data.regBirth} ${data.regPrefix} 
    ${data.regSex} ${data.regNation} 
    ${data.regName} ${data.regLastname} 
    ${data.regEname} ${data.regElastname} 
    ${data.regPhone} ${data.regEmail} ${data.regImg} `;
...
export default EditRegisterFormModal;
