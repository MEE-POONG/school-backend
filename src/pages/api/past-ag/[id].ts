...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    switch (method) {
        case 'GET':
            try {
                const pastAG = await prisma.pastAG.findUnique({
        ...
        case 'PUT':
            try {
                const updatedPastAG = await prisma.pastAG.update({
                    where: { id: id as string },
                    data: req.body,
                });

                res.status(200).json(updatedPastAG);
            } catch (error) {
        ...
        case 'DELETE':
            try {
                const deletedPastAG = await prisma.pastAG.delete({
                    where: { id: id as string },
                });

                res.status(200).json(deletedPastAG);
            } catch (error) {
